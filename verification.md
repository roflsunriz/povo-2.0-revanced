...
- 設定画面: 手順、メール本文欄、現在終了日時欄、保存、有効化、一時停止、正確なアラーム、削除を確認
...
1. ホームカードからメール本文を登録し、暗号化保存と入力期限を確認する。
2. 正確なアラームを許可し、`dumpsys alarm` で予約を確認する。
3. 現在の4回目終端（2026-08-31 16:42 JST）で、終了前 foreground service、拒否時再試行、適用成功、次回予約を時系列で確認する。
4. 端末再起動後とセッション失効後に、コードを失わず復旧することを確認する。
5. `adb logcat` にはコード本文、token、個人情報を出力しないことを確認する。
